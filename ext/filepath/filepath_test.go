...
import (
	...
	"github.com/oakwood-commons/celexp/internal/pathnorm"
...
func TestDirFunc_CELIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedNormalized := pathnorm.NormalizeFilePath(tt.expected)
			resultNormalized := pathnorm.NormalizeFilePath(result.Value().(string))
...
func TestDirFunc_WithVariables(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		require.NoError(t, err)
		// Normalize both paths for cross-platform comparison
		expectedNormalized := pathnorm.NormalizeFilePath(tc.expected)
		resultNormalized := pathnorm.NormalizeFilePath(result.Value().(string))
		assert.Equal(t, expectedNormalized, resultNormalized)
	}
}
...
func TestFilepath_CombinedFunctions(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			checkFunc: func(t *testing.T, result interface{}) {
				// Normalize both paths for cross-platform comparison
				expected := pathnorm.NormalizeFilePath("/usr/local/bin")
				actual := pathnorm.NormalizeFilePath(result.(string))
...
func TestJoinFunc_WithVariables(t *testing.T) {
	...
	require.NoError(t, err)
	// Normalize both paths for cross-platform comparison
	expected := pathnorm.NormalizeFilePath("/home/user/documents/readme.txt")
	actual := pathnorm.NormalizeFilePath(result.Value().(string))
	assert.Equal(t, expected, actual)
}
...
func TestJoinFunc_ChainedOperations(t *testing.T) {
	...
	require.NoError(t, err)
	// Normalize both paths for cross-platform comparison
	expected := pathnorm.NormalizeFilePath("/home/user/documents")
	actual := pathnorm.NormalizeFilePath(result.Value().(string))
	assert.Equal(t, expected, actual)
}
