...
// bufferSink is a test DebugSink that captures debug.out output in a buffer.
// A real embedder passes its own writer (e.g. scafctl's *writer.Writer, which
// already satisfies DebugSink); the library tests only need the interface.
type bufferSink struct{ buf *bytes.Buffer }

func (s *bufferSink) DebugOutf(format string, args ...any) {
	fmt.Fprintf(s.buf, format, args...)
	// Emulate the newline a line-oriented writer would add, so tests that
	// TrimSpace the buffer behave the same as before.
	s.buf.WriteByte('\n')
}

// newTestWriter creates a DebugSink for testing that writes to a buffer.
// Returns the sink and the buffer it writes to.
func newTestWriter() (*bufferSink, *bytes.Buffer) {
	buf := &bytes.Buffer{}
...
// typedNilSink is a concrete Sink pointer type used to exercise the typed-nil
// interface hazard: a nil *typedNilSink boxed into a Sink is a non-nil interface.
type typedNilSink struct{ buf *bytes.Buffer }

func (s *typedNilSink) DebugOutf(format string, args ...any) {
	// Would panic on a nil receiver dereference if ever called with s == nil.
	fmt.Fprintf(s.buf, format, args...)
}

// TestSinkUsable covers the guard against the typed-nil interface hazard.
func TestSinkUsable(t *testing.T) {
	// A true nil interface.
	assert.False(t, sinkUsable(nil))

	// A typed-nil pointer boxed into the interface: this is a non-nil interface
	// value (a plain `sink != nil` check would pass), but sinkUsable must treat
	// it as unusable so we never call a method on the nil pointer.
	var tn *typedNilSink
	var asSink Sink = tn
	assert.False(t, sinkUsable(asSink), "typed-nil pointer must be treated as unusable")

	// A real, usable sink.
	assert.True(t, sinkUsable(&bufferSink{buf: &bytes.Buffer{}}))
}

// TestDebugOutFunc_TypedNilSink is the end-to-end regression: evaluating
// debug.out with a typed-nil sink must NOT panic and must silently skip output.
// env.New relies on this nil-safety when no sink is present in context.
func TestDebugOutFunc_TypedNilSink(t *testing.T) {
	var tn *typedNilSink // nil concrete pointer
	debugFunc := DebugOutFunc(tn)

	env, err := cel.NewEnv(debugFunc.EnvOptions...)
	require.NoError(t, err)

	require.NotPanics(t, func() {
		for _, expr := range []string{`debug.out("x")`, `debug.out("x", 1)`} {
...
// TestDebugOutFunc_NilInterface confirms a plain nil Sink is also safe.
func TestDebugOutFunc_NilInterface(t *testing.T) {
	debugFunc := DebugOutFunc(nil)
	env, err := cel.NewEnv(debugFunc.EnvOptions...)
	require.NoError(t, err)
	require.NotPanics(t, func() {
		ast, _ := env.Compile(`debug.out("x", 42)`)
