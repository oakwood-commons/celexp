...
// Library-owned defaults (SEAM 3). These were previously sourced from scafctl's
// settings package; the library owns them so it is self-contained. An embedder
// may override DefaultCacheSize directly or via SetDefaultCostLimit.
const (
	// DefaultCacheSizeConst is the default size for the CEL program cache.
	DefaultCacheSizeConst = 10000
	// DefaultCostLimitConst is the default cost limit for CEL expression
	// evaluation (0 disables cost limiting).
	DefaultCostLimitConst uint64 = 1000000
)
...
var (
	...
	// Aligned with global cache size for consistency. Mutable via
	// SetDefaultCacheSize; seeded from DefaultCacheSizeConst.
	DefaultCacheSize = DefaultCacheSizeConst
...
func init() {
	defaultCostLimit.Store(DefaultCostLimitConst)
}
